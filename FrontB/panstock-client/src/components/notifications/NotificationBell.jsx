/**
 * NotificationBell.jsx — v4
 * Usa selectores individuales (primitivos) para evitar re-renders.
 * NO llama a useNotifications() — eso lo hace AppTopbar.
 */
...
export default function NotificationBell({ onRequestPermission }) {
  ...
  // Test de notificación directo (Notification API primero, SW como fallback)
  ...
  return (
    <>
      ...
      >
        ...
        {supportsNotifications() && (
          <span className="bell-dot" style={{
            background: permission === 'granted' && enabled
              ? '#27AE60'
              : permission === 'denied'
                ? '#C0392B'
                : 'transparent',
        ...
        {urgentCount > 0 && enabled && permission === 'granted' && (
      ...
      <style>{`
        ...
        .bell-dot { position:absolute; bottom:5px; right:5px; width:7px; height:7px;
